import {
  createWorkflow,
  transform,
  when,
  WorkflowResponse,
} from "@medusajs/framework/workflows-sdk"
import {
  updatePromotionsStep,
  useQueryGraphStep,
} from "@medusajs/medusa/core-flows"
import { CartData, OrderData, orderHasLoyaltyPromotion } from "../../utils/promo"
import {
  validateCustomerExistsStep,
  ValidateCustomerExistsStepInput,
} from "../steps/validate-customer-exists"
import { deductPurchasePointsStep } from "../steps/deduct-purchase-points"
import { addPurchaseAsPointsStep } from "../steps/add-purchase-as-points"
import { getCartLoyaltyPromoStep } from "../steps/get-cart-loyalty-promo"

type HandleOrderPointsWorkflowInput = {
  order_id: string
}

const handleOrderPointsWorkflow = createWorkflow(
  "loyalty.handle-order-points",
  function ({ order_id }: HandleOrderPointsWorkflowInput) {
    const { data: orders } = useQueryGraphStep({
      entity: "order",
      fields: [
        "id",
        "customer.*",
        "total",
        "cart.*",
        "cart.promotions.*",
        "cart.promotions.rules.*",
        "cart.promotions.rules.values.*",
        "cart.promotions.application_method.*",
      ],
      filters: {
        id: order_id,
      },
      options: {
        throwIfKeyNotFound: true,
      },
    })

    validateCustomerExistsStep({
      customer: orders[0].customer,
    } as ValidateCustomerExistsStepInput)

    const loyaltyPointsPromotion = getCartLoyaltyPromoStep({
      cart: orders[0].cart as unknown as CartData,
    })

    const loyaltyPromotionAmount = transform(
      { loyaltyPointsPromotion },
      (data) => Number(data.loyaltyPointsPromotion?.application_method?.value || 0)
    )

    when(orders, (data) => orderHasLoyaltyPromotion(data[0] as unknown as OrderData)).then(() => {
      deductPurchasePointsStep({
        customer_id: orders[0].customer!.id,
        amount: loyaltyPromotionAmount,
        order_id: orders[0].id,
      })

      updatePromotionsStep([
        {
          id: loyaltyPointsPromotion.id,
          status: "inactive",
        },
      ])
    })

    when(orders, (data) => !orderHasLoyaltyPromotion(data[0] as unknown as OrderData)).then(() => {
      addPurchaseAsPointsStep({
        customer_id: orders[0].customer!.id,
        amount: orders[0].total,
        order_id: orders[0].id,
      })
    })

    return new WorkflowResponse(void 0)
  }
)

export default handleOrderPointsWorkflow
