...
export class ShipStationClient {
  ...
  private async sendRequest(
    ...
    try {
      ...
      throw new ShipStationApiError({
        ...
        upstream:
          String(networkErr?.cause || "").slice(
            0,
            ShipStationClient.UPSTREAM_SNIPPET_LIMIT,
          ) || undefined,
      });
    }
    ...
    if (!contentType?.includes("application/json")) {
      const bodyText = (await resp.text()).slice(
        0,
        ShipStationClient.UPSTREAM_SNIPPET_LIMIT,
      );
      throw new ShipStationApiError({
  ...
  async getTrackingInfo(labelId: string): Promise<TrackingInfo> {
    return await this.sendRequest(
      `/tracking?label_id=${encodeURIComponent(labelId)}`,
    );
  }
