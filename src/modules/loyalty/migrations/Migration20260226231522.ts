...
export class Migration20260226231522 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_LOYALTY_POINT_CUSTOMER_ID" ON "loyalty_point" ("customer_id") WHERE deleted_at IS NULL;`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_loyalty_point_deleted_at" ON "loyalty_point" ("deleted_at") WHERE deleted_at IS NULL;`);

    this.addSql(`create table if not exists "loyalty_transaction" ("id" text not null, "customer_id" text not null, "loyalty_point_id" text null, "order_id" text null, "cart_id" text null, "type" text not null, "points" integer not null, "balance_after" integer not null default 0, "reason" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "loyalty_transaction_pkey" primary key ("id"));`);
