...
export class Migration20260302153524 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "wishlist_item" ("id" text not null, "variant_id" text not null, "wishlist_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "wishlist_item_pkey" primary key ("id"));`);
    ...
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_wishlist_item_unique_variant_per_wishlist" ON "wishlist_item" ("wishlist_id", "variant_id") WHERE deleted_at IS NULL;`);
