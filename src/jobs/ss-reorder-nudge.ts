...
export default async function ssReorderNudgeJob(container: MedusaContainer) {
  ...
  const templateId = (process.env.SS_SENDGRID_TEMPLATE_REORDER_NUDGE || "").trim()
  ...
  // Find fulfillments created ~21 days ago
  const { data: fulfillments } = await query.graph({
    ...
    pagination: { take: DEFAULT_LIMIT, skip: 0 },
  })
  ...
  for (const f of eligible) {
    ...
    const items = (order.items || []).map((item) => {
      ...
      return {
        ...
        price: formatCurrency(item.unit_price, order.currency_code)?.replace("$", "") ?? "0.00",
        unit_price_display: formatCurrency(item.unit_price, order.currency_code),
  ...
  if (sentCount > 0) {
    logger.info(
      `[ss-reorder-nudge] Sent ${sentCount} reorder nudge(s).`
    )
  }
}
