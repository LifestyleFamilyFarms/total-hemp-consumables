...
When you install a Medusa application, you get a fully-fledged commerce platform with a Framework for customization. The Medusa application's commerce features are built around [Commerce Modules](https://docs.medusajs.com/commerce-modules), which are available out-of-the-box. These features include cart-management capabilities.

Medusa's [Notification Module](https://docs.medusajs.com/infrastructure-modules/notification) allows you to send notifications to users or customers, such as password reset emails, order confirmation SMS, or other types of notifications.
...
You will first be asked for the project's name. Then, when asked whether you want to install the [Next.js Starter Storefront](https://docs.medusajs.com/nextjs-starter), choose "Yes."
...
To send the cart-abandonment emails, you will use SendGrid. Medusa provides a [SendGrid Notification Module Provider](https://docs.medusajs.com/infrastructure-modules/notification/sendgrid) that you can use to send emails.

Alternatively, you can use [other Notification Module Providers](https://docs.medusajs.com/infrastructure-modules/notification#what-is-a-notification-module-provider) or [create a custom provider](https://docs.medusajs.com/references/notification-provider-module).
...
- [updateCartsStep](https://docs.medusajs.com/references/medusa-workflows/steps/updateCartsStep): Update the cart to store the last notification date.
...
export const sendAbandonedNotificationsStep = createStep(
  ...
  async (input: SendAbandonedNotificationsStepInput, { container }) => {
    ...
    const notificationData = input.carts.map((cart) => ({
      to: cart.email!,
      channel: "email", 
      template: process.env.SENDGRID_TEMPLATE_ABANDONED_CART || "",
      data: {
        customer: {
...
<body>
    <div class="container">
        <div class="header">Hi {{customer.first_name}}, your cart is waiting! ðŸ›ï¸</div>
...
```plain
SENDGRID_TEMPLATE_ABANDONED_CART=your-sendgrid-template-id
```
...
To run a task at a scheduled interval, you can use a [scheduled job](https://docs.medusajs.com/learn/fundamentals/scheduled-jobs). A scheduled job is an asynchronous function that the Medusa application runs at the interval you specify during the Medusa application's runtime.
...
1. An asynchronous function that holds the job's logic. The function receives the [Medusa container](https://docs.medusajs.com/learn/fundamentals/medusa-container) as a parameter.
...
In the scheduled job function, so far you resolve the [Logger](https://docs.medusajs.com/learn/debugging-and-testing/logging) to log messages, and [Query](https://docs.medusajs.com/learn/fundamentals/module-links/query) to retrieve data across modules.
...
And in the [Next.js Starter Storefront](https://docs.medusajs.com/nextjs-starter)'s directory (that you installed in the first step), start the storefront with the following command:
...
- [Implement Product Reviews](https://docs.medusajs.com/how-to-tutorials/tutorials/product-reviews).
- [Implement Wishlist](https://docs.medusajs.com/plugins/guides/wishlist).
- [Allow Custom-Item Pricing](https://docs.medusajs.com/examples/guides/custom-item-price).

If you are new to Medusa, check out the [main documentation](https://docs.medusajs.com/learn), where you will get a more in-depth learning of all the concepts you have used in this guide and more.
